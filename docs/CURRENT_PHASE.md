...
- [x] 来源目录与连接器完成（52个登记来源：39个启用、13个禁用候选；新增10栏目；39来源fixture通过；2026-09-09本机全量试采37/39成功，单源失败隔离）
- [x] 去重、分类、静态导出完成
- [x] Vue 3移动端PWA主要页面完成
- [x] 搜索、筛选、收藏和关注完成
- [x] GitHub Actions分组定时采集代码完成（快速每4小时、政策/药企每6小时、学术/医院每天2次；五组扩源后重新观察连续7次）
...
决策：按用户确认增加学术、中国政策、国内外药企和国内医院的内容覆盖，增加10个官方栏目；首页新增五个重点入口，今日必读每来源最多两条。中文摘要服务明确暂不实施，不改变原 Provider。

原因：来源总量增加后仍存在国内医院、国内药企覆盖不足及重点展示集中于少数监管源的问题。

影响的需求ID：V1-F-003、V1-F-004、V1-F-008、V1-F-009、V1-F-013、V1-F-014；不扩展V2/V3。

迁移或回滚方式：Article契约仍为1.0，收藏和稳定ID算法不变；新增pharma/hospital调度组，旧institutional参数兼容映射到两组。新增来源可逐项禁用；中文路径规范化输出百分号编码URI，避免生成数据违反原有URI契约。
...
日期：2026-09-08
决策：V1来源扩展为29个启用来源和13个禁用候选，按`rapid`、`policy`、`research`、`institutional`四组错峰采集；全局并发默认4且同域名串行。
原因：覆盖国内外学术、政策、医院和药企，同时按信息时效控制请求量；对HTTP 403/405/412、客户端挑战、动态私有接口和不稳定TLS不采取绕过措施。
影响的需求ID：V1-F-003、V1-F-008、V1-F-009、V1-F-013、V1-F-014、V1-F-015、V1-A-001、V1-A-009、V1-A-010。
迁移或回滚方式：不传`--groups`仍采集全部启用来源；可将新增来源逐项设为`enabled: false`，或回退工作流为单一四小时任务；Article 1.x契约、前端仓储边界和静态文件路径保持不变。
